...
const filterHandler = (event) => {
  // const {filter}= event.target.dataset; روش اول
  // const filter = event.target.dataset.filter;(روش دوم)
...
const searchPriceHandler = (event) => {
  ...
  products.forEach((product) => {
    ...
    const price = +productPrice.split("$").join("").trim();
    if (!searchPrice) {
